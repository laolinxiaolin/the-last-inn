...
## Prototype — the levels (environments as distinct stages)

- **Each space is now its own level, not one reused backdrop.** The prototype previously swapped the same `bg_inn` everywhere; the design's stages now get the room they name (docs/14 §3): the inn interior, the cellar, the wall, the night outside, and the door.
- **The inn** (bg_inn, rebuilt at 1080p) is the stage: POV behind the bar across the hearth and the window, the museum wall half-remembered behind the guests, the two-light rule (amber firelight + cold windowlight) kept from docs/14.
- **The cellar** (`bg_cellar`) — night three's capstone, now where the night actually opens. Kegs breathing, a ladder, a single lamp's pool on the stone, and the locked door at the far end leaking its wrong, cold seam. Wired to both `_start_night_three` and the closing batch decision.
- **The wall** (`bg_wall`) — the living museum given a room: her sword, the borrowed sword too big for a boy, a road hat, a coin too new, the bell key, Grib's candle, a broken lute — every keepsake the player put there. Wired to the wall walk.
- **The window** (`bg_exterior`) — the night outside, deep winter: the valley, the treeline, the last warm lights, one figure far down the road. Wired to "listen to the night" and the dawn window.
- **The door** (`bg_door`, rebuilt) — the threshold to the world, ajar on the dark, the lamp pooling warm at the flags. Still the courier's return, now also the door past the cellar.
- **The title** (`bg_title`, rebuilt) — the inn from the road: one warm window burning in a deep winter night, snow falling, the road walked to the last light.
- **All verified headless**: clean import (Godot regenerated the `.import` files for the four new SVGs), the full world-state smoke suite passes, and the 36-shot autoplay runs error-free. Vision review was unavailable this pass (the image channel dropped mid-session), so the SVGs were authored to deterministic geometry and will be confirmed by eye in the running game.
